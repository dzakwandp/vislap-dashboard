      ...
      <v-card class="mx-auto pa-8">
        ...
        >
          ...
          <div class="d-flex flex-row mb-4 align-center">
            ...
            >
              ...
              <v-tooltip activator="parent" location="bottom"
                >Gambar harus dalam format JPG/PNG</v-tooltip
              >
            </v-btn>
          </div>
          <!-- for activate edit -->
          <div v-if="!editable" class="text-center">
            <v-btn
              class="text-body-2 text-white mr-8"
            ...
            <v-btn
              ...
              color="red-lighten-2"
              :loading="loadingdel"
              @click="(loadingdel = true), deleteProduct()"
              >Delete</v-btn
            >
          </div>
          <!-- for submit edit -->
          <div v-if="editable" class="text-center">
            <v-btn
              ...
              :loading="loadingup"
              @click="(loadingup = true), updateProduct()"
              >Simpan</v-btn
            >
            <v-btn
              class="text-body-2 text-white"
...
export default {
  ...
  methods: {
    async getProduct() {
      try {
        const prod = await axios.get(
          useEnvStore().apiUrl + "products/" + this.$route.params.id,
          {
            headers: {
    ...
    deleteProduct() {
      axios
        .delete(useEnvStore().apiUrl + "products/" + this.$route.params.id, {
          headers: {
            Authorization: "Bearer " + useAuthStore().accessToken,
          },
        })
